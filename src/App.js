...
function Dashboard() {
  ...
  // ✅ REMOVED localStorage plan poisoning
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.log("SUBSCRIPTION STATUS:", subRes.data);
  ...
  const refreshLatestReport = () => {
    ...
    try {
      ...
      if (!saved) {
        ...
        setError(
          "No completed assessment found. Please complete the HIPAA assessment first."
        );
